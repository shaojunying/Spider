...
def get_tournament_data(year: str = "2025") -> Dict:
    ...
    for i in range(0, 2):
        num = int(year[2:])
        season_id = f'{num+i-1}{num+i}'
        # Build the curl command
        curl_command = f"""
        curl 'https://bw.biathlonresults.com/modules/sportapi/api/Events?SeasonId={season_id}' \
          -H 'Accept: application/json, text/plain, */*' \
          -H 'Accept-Language: en-US,en;q=0.9,zh-CN;q=0.8,zh;q=0.7,la;q=0.6,und;q=0.5' \
          -H 'Connection: keep-alive' \
          -H 'DNT: 1' \
          -H 'Origin: https://www.biathlonworld.com' \
          -H 'Referer: https://www.biathlonworld.com/' \
        ...
        try:
            logging.info(f"Fetching tournament data for {season_id}")
            ...
            if result.returncode == 0:
                ...
                logging.info(f"Successfully fetched data for {season_id}")
            ...
            logging.error(f"Failed to execute curl command for {season_id}: {str(e)}")
            raise
        except json.JSONDecodeError as e:
            logging.error(f"Failed to parse JSON response for  {season_id}: {str(e)}")
            raise
        ...
        for event in results:
            if isinstance(event, dict):
                flattened_event = {
                    ...
                    'Trimester': event.get('Trimester', ''),
                    'EventId': event.get('EventId', ''),
                    'StartDate': event.get('StartDate', ''),
                    'EndDate': event.get('EndDate', ''),
                    'FirstCompetitionDate': event.get('FirstCompetitionDate', ''),
                    'Description': event.get('Description', ''),
                    'EventSeriesNr': event.get('EventSeriesNr', ''),
                    'ShortDescription': event.get('ShortDescription', ''),
                    'Altitude': event.get('Altitude', ''),
                    'OrganizerId': event.get('OrganizerId', ''),
                    'Organizer': event.get('Organizer', ''),
                    'Nat': event.get('Nat', ''),
                    'NatLong': event.get('NatLong', ''),
                    'MedalSetId': event.get('MedalSetId', ''),
                    'EventClassificationId': event.get('EventClassificationId', ''),
                    'Level': event.get('Level', ''),
                    'UTCOffset': event.get('UTCOffset', ''),
                    'IsActual': event.get('IsActual', ''),
                    'IsCurrent': event.get('IsCurrent', ''),
                    'EventStatusId': event.get('EventStatusId', ''),
